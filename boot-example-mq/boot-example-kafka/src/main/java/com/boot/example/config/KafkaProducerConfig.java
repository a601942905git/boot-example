...
public class KafkaProducerConfig {
    ...
    public ProducerFactory<?, ?> kafkaProducerFactory(KafkaProperties kafkaProperties) {
        ...
        // 请求被认为完成前生产者要求leader应收到指定数量的ack
        producerProperties.put(ProducerConfig.ACKS_CONFIG, "all");
        // 重试次数，如果max.in.flight.requests.per.connection不设置为1，会导致发送消息错乱
        producerProperties.put(ProducerConfig.RETRIES_CONFIG, "3");
        // 缓冲池大小，当消息发送速度超过发送给broker的速度，会导致缓冲池耗尽并对生产者进行max.block.ms时间阻塞，超时后会抛出异常
